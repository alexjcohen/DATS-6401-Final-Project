...
document.getElementById("income-change-over-time").innerHTML = "";

// get variables based on inputs
var yrname = document.getElementById('input_value_income')
var income_yr = "income_" + yrname.options[yrname.selectedIndex].text;
...
var canvas = d3.select("#income-change-over-time").append("svg")
...
canvas.append("text")
	...
	.attr("text-anchor", "middle")
	.text("Change in Median Income between 2010 and 2017")	
...
var tip1 = d3.tip().attr("class", "d3-tip")
	.html(function(d){
		...
		info += "<strong>2010 Median Annual Income: </strong><span style='color: grey'>" + d3.format("$,.0f")(d.income_2010) + "</span></br></br>";
...
var tip2 = d3.tip().attr("class", "d3-tip")
	.html(function(d){
		...
		var info = "<strong>City: </strong><span style='color: grey'>" + d.Geography + "</span> </br> </br>";
		info += "<strong>2017 Median Annual Income: </strong><span style='color: grey'>" + d3.format("$,.0f")(d.income_2017) + "</span></br></br>";
		info += "<strong>Region: </strong><span style='color: grey'>" + d.region + "</span>";
...
d3.csv("data/income.csv").then(function(data){
	...
	lines.enter()
		...
		.merge(lines)
		.transition(t)
			.attr("y1", function(d){return y(d.income_2010)})
			.attr("y2", function(d){return y(d.income_2017)})
	...
	var circles2010 = canvas.selectAll("circle")
		...
		circles2010.enter()
			...
			.transition(t)
				...
				.attr("cy", function(d) {return y(d.income_2010)} )
				.attr("fill", palate[1])	
				.attr("stroke", "grey")
				.attr("stroke-width", "1")
				.attr("r", "8")
	...
	var circles2017 = canvas.selectAll("circles")
		...
		circles2017.enter()
			...
			.transition(t)
				.attr("cx", function(d){return x(d.Geography)})
				.attr("cy", function(d) {return y(d.income_2017)} )
				.attr("fill", palate[3])	
				.attr("stroke", "grey")
				.attr("stroke-width", "1")
				.attr("r", "8")
...
// remove previous contents
document.getElementById("income-vs-payment").innerHTML = "";

// get variables based on inputs
var yrname = document.getElementById('input_value_income')
var type   = document.getElementById('input_value_income_category')
var income_yr = "income_" + yrname.options[yrname.selectedIndex].text;
...
// create canvas
var canvas2 = d3.select("#income-vs-payment").append("svg")
			.attr("height", height + margin.top + margin.bottom)
			.attr("width", width + margin.left + margin.right)
		.append("g")
			.attr("transform", "translate(" + margin.left + ", " 
...
canvas2.append("text")
		...
		.style("text-decoration", "underline")
		.attr("text-anchor", "middle")
		.text("Median Income vs. Median Housing/Rent Price")
...
var tip = d3.tip().attr("class", "d3-tip")
	.html(function(d){
		...
		info += "<strong>Median Income: </strong><span style='color: grey'>" + d3.format("$,.0f")(d[income_yr]) + "</span><br /><br />";
...
d3.csv("data/income.csv").then(function(data){
	...
	// adding in the axes
	var x = d3.scaleLinear()
		.domain([0, d3.max(data, function(d){return d[income_yr]})]) 
		.range([0, width]);
	...
	$("#input_value_income")
		.on("change", function(){
			year = document.getElementById('input_value_income')
			year = year.options[year.selectedIndex].text;
			type = document.getElementById('input_value_income_category')
			type = type.options[type.selectedIndex].value
			update(data, year, type);
	});	
	
	$("#input_value_income_category")
		.on("change", function(){
			year = document.getElementById('input_value_income')
			year = year.options[year.selectedIndex].text;
			type = document.getElementById('input_value_income_category')
	...
	function update(data, year, type) {
		...
		circles.enter()
			...
			.transition(t)
					...
					.attr("cx", function(d){return x(d[income_yr])}) 
					.attr("r", 7)
					.attr("stroke", "grey")
					.attr("stroke-width", "2");
